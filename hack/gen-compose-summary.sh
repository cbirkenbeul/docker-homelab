...
do
    IFS=$'\n' readarray -t chart_data <<< $(yq eval '.name, .description, .blog_url, .video_url' "$i")
    chart_name="${chart_data[0]}"
    chart_description="${chart_data[1]}"
    chart_blog_url="${chart_data[2]}"
    chart_video_url="${chart_data[3]}"
    echo "| [${chart_name}](/../tree/master/compose-files/applications/${chart_name}) | ${chart_description} | ${chart_blog_url} | ${chart_video_url} |" >> "${compose_summary_file}"
done
